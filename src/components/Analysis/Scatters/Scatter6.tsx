    ...
    return (
      ...
      >
        <div className='custom-tooltip'>
          ...
          <p className='label'>{`Player:Robert Lewandowski`}</p>
          <p className='label'>{`Goal/Game : 5`}</p>
          <p className='label'>{`Succesful Dribbling/Game: 2`}</p>
          <p className='label'>{`Team : Bayern Munich'`}</p>
          <p className='label'>{`Rank Pass per Game : 4`}</p>
          <p className='label'>{`Rank Key Passes Per Game : 2`}</p>
...
export default function Scatter6() {
    ...
    <div className={classes.scatterRoot}>
      <Paper className={classes.scatterPaper}>
        <ResponsiveContainer width='100%' height='100%'>
          ...
          >
            {/* <CartesianGrid /> */}
            <XAxis type='number' dataKey='x' name='Key Passes/Game'>
              <Label value='Key Passes/Game' offset={-15} position='insideBottom' />
            </XAxis>
            <YAxis offset={10} type='number' dataKey='y' name='Passes Total'>
              <Label value='Passes Total' angle={-90} position='insideLeft' />
            </YAxis>

            <Tooltip cursor={{ strokeDasharray: '3 3' }} content={CustomTooltip} />
            <Scatter data={data} fill='#686868' shape='diamond' />
          </ScatterChart>
        </ResponsiveContainer>
      </Paper>
    </div>
