    ...
    return (
      ...
      >
        <div className='custom-tooltip'>
          ...
          <p className='label'>{`Player:Toni Kroos`}</p>
          <p className='label'>{`Goal/Game : 5`}</p>
          <p className='label'>{`Succesful Dribbling/Game: 2`}</p>
          <p className='label'>{`Team :Germany'`}</p>
          <p className='label'>{`Rank Duels Won Per Game : 4`}</p>
...
export default function Scatter9() {
    ...
    <div className={classes.scatterRoot}>
      <Paper className={classes.scatterPaper}>
        <ResponsiveContainer width='100%' height='100%'>
          ...
          >
            {/* <CartesianGrid /> */}
            <XAxis type='number' dataKey='x' name='Duels Total Game'>
              <Label value='Duels Total Game' offset={-15} position='insideBottom' />
            </XAxis>
            <YAxis offset={10} type='number' dataKey='y' name='Duels Won/Game'>
              <Label value='Duels Won/Game' angle={-90} position='insideLeft' />
            </YAxis>

            <Tooltip cursor={{ strokeDasharray: '3 3' }} content={CustomTooltip} />
            <Scatter data={data} fill='#434343' shape='diamond' />
          </ScatterChart>
        </ResponsiveContainer>
      </Paper>
    </div>
